...
**零工具依赖。** 任何串口终端都能直接和设备交互，不需要专用上位机。对于一个替代封闭厂商模块的工具，开放的第一步就是不写代码也能上手。
...
JSON 盯着看就能理解，二进制需要翻协议文档对着字节偏移量数。

**向前兼容。** 加新字段不破坏旧解析器，无需协议版本号协商。

**性能够用。** 日常调试场景（115200bps 下约 115 帧/秒）完全覆盖。高负载 CAN 总线下瓶颈是 USB CDC 波特率而非 JSON 开销。如需 3000+ 帧/秒持续记录，建议提高波特率或考虑专业 CAN 分析仪。

**权衡。** JSON 每帧约 100 字节 vs 二进制约 14 字节。这是一个调试工具，优先级是「先让人理解，再让机器优化」。
